...
int main(){
	...
	do{
		...
		scanf("%d",&choice);
		switch(choice){
			//1.Nhap so luong va gia tri cho cac phan tu
			int i,n,arr[100];
			case 1:
			...
			case 4:
				...
				printf(" \n");
				break;
			//5.Tim gia tri lon thu 2 (khong sap xep)
			int max,max_2;
			case 5:
		     	max = arr[0];
		    	for(i=0;i<n;i++){
				...
				printf(" \n");
		    	break;
			//6.Tim gia tri nho thu 2 (khong sap xep)
			int min,min_2;
			case 6:
		     	min = arr[0];
		        for(i=0;i<n;i++){
			...
			case 8:
		        ...
		        printf(" \n");
				break;
			//9.Sap xep mang tang dan (sap xep chen)
			int key,j;
			case 9:
				for(i=1;i<n;i++){
					key=arr[i];
                ...
                printf(" \n");
				break;
			//10.Sap xep mang giam dan (sap xep chen)
			case 10:
				for(i=1;i<n;i++){
					key=arr[i];
					j=i-1;
                ...
                printf(" \n");
				break;
	        //11.Tim kiem phan tu(Tim kiem nhi phan)
			int x,start,end,mid;
			case 11:
	        	start=0;
	        	end=n;
	            ...
	            printf(" \n");
				break;
			//12.Xoa phan tu trung lap va in phan tu doc nhat
			int check,m,b[100];
			case 12:
				check=0;
				for(i=0;i<n;i++){
                ...
                printf(" \n");
				break;
			//13.Sap xep va hien thi so chan dung truoc, so le dung sau
			int temp;
			case 13:
				for(i=0;i<n;i++){
                    for(j=0;j<n-i-1;j++){
                ...
                printf(" \n");
				break;
			//14.Sap xep va hien thi so le dung truoc, so chan dung sau
			case 14:
				for(i=0;i<n;i++){
                    for(j=0;j<n-i-1;j++){
                        if((arr[j]%2<arr[j+1]%2) || (arr[j]%2==arr[j+1]%2 && arr[j]>arr[j+1])){ 
